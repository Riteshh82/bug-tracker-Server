...
const getBugs = async (req, res, next) => {
  try {
    const { project, priority, status, assignedTo, search, page = 1, limit = 20, isDeleted } = req.query;
    ...
    res.json({ success: true, total, page: Number(page), pages: Math.ceil(total / limit), bugs });
  } catch (err) { next(err); }
};
...
const createBug = async (req, res, next) => {
  try {
    const screenshots = req.files ? req.files.map(f => ({ url: `/uploads/${f.filename}`, filename: f.originalname })) : [];

    // Parse tags: accept comma-separated string OR array
    let tags = req.body.tags;
    if (typeof tags === 'string') tags = tags.split(',').map(t => t.trim()).filter(Boolean);

    const bugData = { ...req.body, createdBy: req.user._id, screenshots, tags: tags || [] };
    const bug = await Bug.create(bugData);

    await ActivityLog.create({
      entity: 'Bug', entityId: bug._id, action: 'created',
      performedBy: req.user._id, metadata: { bugId: bug.bugId, title: bug.title },
    });

    // Notify assigned user
    if (bug.assignedTo && bug.assignedTo.toString() !== req.user._id.toString()) {
      await Notification.create({
        recipient: bug.assignedTo, sender: req.user._id, type: 'bug_assigned',
        message: `You have been assigned bug ${bug.bugId}: ${bug.title}`,
        entityType: 'Bug', entityId: bug._id,
      });
    }
...
const updateBug = async (req, res, next) => {
  try {
    ...
    trackedFields.forEach(field => {
      if (req.body[field] !== undefined && String(bug[field]) !== String(req.body[field])) {
        historyEntries.push({ field, oldValue: bug[field], newValue: req.body[field], changedBy: req.user._id });
      }
    });
    ...
    if (changedFields) {
      await ActivityLog.create({
        entity: 'Bug', entityId: bug._id, action: 'updated',
        performedBy: req.user._id, metadata: { fields: changedFields },
      });
    }

    // Notify creator on status change (not if they did it themselves)
    if (req.body.status && req.body.status !== previousStatus && createdById && createdById !== req.user._id.toString()) {
      await Notification.create({
        recipient: bug.createdBy, sender: req.user._id, type: 'status_changed',
...
const deleteBug = async (req, res, next) => {
  try {
    await Bug.findByIdAndUpdate(req.params.id, { isDeleted: true, deletedAt: new Date() });
...
const restoreBug = async (req, res, next) => {
  try {
    await Bug.findByIdAndUpdate(req.params.id, { isDeleted: false, deletedAt: null });
...
module.exports = { getBugs, createBug, getBug, updateBug, deleteBug, restoreBug, getBugActivity };
